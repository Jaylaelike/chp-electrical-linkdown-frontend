<template>
  ...
  <div class="q-pb-md">
    <q-table title="ตารางข้อมูล Link NT Down ศูนย์ชุมพร" :rows="rows" :columns="columns" row-key="name" :filter="filter" dark
      color="primary">
...
const fetchData = () => {
  ...
  const apiUrl = import.meta.env.VITE_API_URL + "/api/nt/" + `${startDate}` + "/" + `${endDate}`
